...
//  Project 10
//
//  Created by Tuğşad Şen on 26.03.2022.
//
...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @objc func addNewPerson(isCamera: Bool) {
...
//        present(picker, animated: true)
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let deleteButton = UIAlertAction(title: "Delete", style: .default) {[weak self] _ in
            self?.delete(person: person, index: indexPath)
        }
...
//        present(ac, animated: true)
    ...
    @objc func delete(person: Person, index: IndexPath) {
        people.remove(at: index.item)
        collectionView.reloadData()
    }
    
    @objc func rename(person: Person) {
